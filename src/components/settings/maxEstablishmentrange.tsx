...
import { update_establishment_max_distance, clearEstablishmentUpdateState } from "@/redux/slices/settinsSlice/distanceSlice/updateEstablishmentMaxDistance";
import toast from "react-hot-toast";
import Loader from "../loader/Loader";
import "./MaxDistance.css";

const MaxEstablishmentDistanceRange = () => {
    ...
    const updateMaxDistance = useSelector(
        (state: RootState) => state.UPDATE_ESTABLISHMENT_DISTANCE,
    );

    useEffect(() => {
        if (maxDistanceData?.isSuccess) {
            dispatch(clearEstablishmentUpdateState());
            dispatch(clear_max_distance_state());
            const max = maxDistanceData?.data?.serchHotelsUnder || 0;
            setRangeValue(max);
        }
    }, [maxDistanceData]);
    ...
    const handleRangeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        debounceTimeout.current = setTimeout(() => {
            dispatch(update_establishment_max_distance({ maximum_distance: newRangeValue }));
        }, 100);
    };
    ...
    return (
        <div>
            ...
            >
                <p> Maximum Establishment Distance Range</p>
...
export default MaxEstablishmentDistanceRange;
