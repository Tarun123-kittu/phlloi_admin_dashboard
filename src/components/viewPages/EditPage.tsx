...
const EditPageContent: React.FC<ChangePasswordModalProps> = ({ sectionId, headingId }) => {
    ...
    const [content, setContent] = useState<PagesArray | undefined>({
        _id: "",
        sectionId,
        title: "",
        content: "",
    ...
    useEffect(() => {
        if (textList?.length > 0) {
            const selectedContent: PagesArray | undefined = textList.find((el) => el?._id === headingId)
    ...
    return (
        <>
            {(
                <div>
                    <div className='mb-4'>
                        <img onClick={() => router.back()} src="https://img.icons8.com/?size=100&id=AO1h97ca7e0A&format=png&color=FFFFFF" alt="back button" width={30} height={30} style={{ cursor: "pointer" }} />
                    ...
                    {section_details?.isSuccess && <div className="">
                        ...
                        <div className="mb-6">
                            <label htmlFor="section-name" className="block mb-2 text-2xl font-medium text-white dark:text-white">{content?.title}</label>
                        </div>
                        {Array.isArray(textList) && textList?.length > 0 && textList.map((list, index) => (
                            <>
                                {list?._id === headingId && <div key={index} className='mb-4 mt-3 bg-cardBg p-4 rounded'>
                                    <div className='flex justify-end gap-3' title='View Page'>
                                        <a href={`https://www.phloii.com/${list.slug}`} target="_blank" rel="noopener noreferrer">
                                    ...
                                    <EditTextEditor setTextList={setTextList} index={index} textList={textList} disable={false} newPage={false}/>
                                </div>}
                            </>
                        ))}
                        <div className='flex gap-3'>
                            {!update_section_data?.isLoading ? <button onClick={(e) => handleSave(e)} type="button" className="text-center text-sm font-medium text-black rounded  bg-gradient-to-r from-[#fbb90d] to-[#22ebff] w-20 h-10 ml-auto">Update</button> :
