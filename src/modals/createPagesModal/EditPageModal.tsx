...
const EditPagesModal: React.FC<ChangePasswordModalProps> = ({ setIsOpen, isOpen, sectionId }) => {
    ...
    useEffect(() => {
        ...
        if (update_section_data?.isError) {
            toast.error("Error while creating new page,Please try again later")
            dispatch(clear_update_section())
        }
    }, [update_section_data])
                    ...
                    {section_details?.isSuccess && <div className="">
                        ...
                        <div className=" bg-cardBg rounded-lg shadow dark:bg-gray-700 p-8 scroll-smooth overflow-y-auto scrollbar scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-200" style={{ maxHeight: "98vh", maxWidth: "750px", width: "100%" }}>
                            ...
                            <div className='mb-4 mt-4'>
                                <div className="mb-6">
                                    <label htmlFor={`page-name-${textList.length-1}`} className="block mb-2 text-sm font-medium text-white dark:text-white">Page name</label>
                                    <input
                                        type="text"
                                        id={`page-name-${textList.length -1}`}
                                        value={textList[textList.length-1]?.title}
                                        onChange={(e) => handleSetValues(e, textList.length-1, "title")}
                                        className="bg-black text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    />
                                </div>
                                <div className='text-white'>

                                    <EditTextEditor setTextList={setTextList} index={textList.length-1} textList={textList} disable={false} newPage={true} />
                                </div>
                            </div>
                            <div className='flex gap-3'>
                                ...
                                {!section_details?.isLoading ? <button onClick={(e) => handleSave(e)} type="button" className="text-center text-sm font-medium text-black rounded  bg-gradient-to-r from-[#fbb90d] to-[#22ebff] w-full">Create</button> :
